...
export class ManageIndividualBeverageSalesComponent {
  ...
  machineIdSuggestions: string[] = [
    ...
    'VM-011', 'VM-012', 'VM-013', 'VM-014', 'VM-015'
  ];
  ...
  applyFilters() {
    this.salesData = this.originalSalesData.filter((sale) => {
      ...
      const matchesMachineId =
        ...
        sale.machineId.toLowerCase().includes(this.filterForm.machineId.toLowerCase());

      // Company name filter
      const matchesCompanyName =
        !this.filterForm.companyName ||
        sale.companyName.toLowerCase().includes(this.filterForm.companyName.toLowerCase());
      ...
      return matchesBeverage && matchesMachineId && matchesCompanyName && matchesDateRange;
    });
  ...
  get selectedBeverage(): string {
    return this.filterForm.beverage === 'All' ? 'All Beverages' : this.filterForm.beverage;
  }
}